...
1. **Compile time**: tobari adds missing imports to the compile-phase `importcfg` using archive paths obtained by `collectExportPaths` (`go list -export -json`). The compiler only needs type information (export data) from these archives, so fingerprint mismatches do not occur.
