...
func handleCompile(ctx context.Context, toolPath string, args []string, embedCode bool) error {
	...
	if pkgName == "main" {
		...
		suppDeps, err := cover.ReadSuppDeps(importcfgPath)
		if err != nil {
			return fmt.Errorf("failed to read supplementary deps: %w", err)
		}
		hookFile, err := generateMainHook(embedCode, suppDeps)
		if err != nil {
...
func generateMainHook(embedCode bool, suppDeps string) (string, error) {
	...
	b.WriteString("// Code generated by tobari.\npackage main\n\nimport (\n\t_ \"github.com/goccy/tobari\"\n\t_ \"unsafe\"\n)\n\n")

	// Always include AddSupplementaryDeps linkname (even if suppDeps is empty,
	// it's a no-op and the linkname is needed for the symbol to resolve).
	b.WriteString("//go:linkname _tobari_addSuppDeps github.com/goccy/tobari/internal/tobari.AddSupplementaryDeps\n")
	b.WriteString("func _tobari_addSuppDeps(string) bool\n\n")
	b.WriteString(fmt.Sprintf("var _ = _tobari_addSuppDeps(%q)\n\n", suppDeps))

	if embedCode {
		b.WriteString("//go:linkname _tobari_extractEmbeddedSources github.com/goccy/tobari/internal/tobari.ExtractEmbeddedSourcesIfRequested\n")
		b.WriteString("func _tobari_extractEmbeddedSources()\n\n")
