...
func handleCompile(ctx context.Context, toolPath string, args []string, embedCode bool) error {
	...
	// Inject extraction hook for main package when embed-code is enabled.
	// This is done at compile time (not cover time) because compile always
	// processes main regardless of -coverpkg settings.
	if embedCode && pkgName == "main" {
		extractFile, err := generateExtractHook()
		if err != nil {
			return fmt.Errorf("failed to generate extract hook: %w", err)
		}
		args = append(args, extractFile)
		if err := addMissingImportsToImportcfg(args, []string{"os"}); err != nil {
			return err
		}
	}
...
const extractHookSource = `// Code generated by tobari.
package main
...
func init() {
	if os.Getenv("TOBARI_EXTRACT_SOURCES") != "" {
		_tobari_extractEmbeddedSources()
	}
}
`

func generateExtractHook() (string, error) {
	f, err := os.CreateTemp("", "_tobari_extract_*.go")
	if err != nil {
		return "", fmt.Errorf("failed to create temp file: %w", err)
	}
	if _, err := f.WriteString(extractHookSource); err != nil {
		_ = f.Close()
		return "", fmt.Errorf("failed to write extract hook: %w", err)
	}
