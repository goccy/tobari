...
func Run(ctx context.Context, args []string, embedCode bool) error {
	...
	if len(inputFiles) == 1 && opt.output != "" {
		...
		if shouldEmbed {
			tobariFiles, err := copyInputFilesWithTobariSuffix(inputFiles)
			if err != nil {
				return err
			}
			outputFiles = append(outputFiles, tobariFiles...)
		}
...
// copyInputFilesWithTobariSuffix copies original source files with .tobari suffix
// so they are not recognized as Go source files but are available for embedding.
func copyInputFilesWithTobariSuffix(inputFiles []string) ([]string, error) {
	tobariFiles := make([]string, 0, len(inputFiles))
	for _, inputFile := range inputFiles {
		content, err := os.ReadFile(inputFile)
		if err != nil {
			return nil, fmt.Errorf("failed to read input file %s for embedding: %w", inputFile, err)
		}
		tobariName := filepath.Base(inputFile) + ".tobari" // e.g., foo.go → foo.go.tobari
		if err := os.WriteFile(tobariName, content, 0o600); err != nil {
			return nil, fmt.Errorf("failed to write tobari file %s: %w", tobariName, err)
		}
		tobariFiles = append(tobariFiles, tobariName)
	}
...
`, pkgcfg.PkgName, tobariPkg)
	...
	// Build the init function that reads each embedded file and calls AddEmbeddedSource
	var addSourceCalls strings.Builder
	for _, inputFile := range inputFiles {
		tobariName := filepath.Base(inputFile) + ".tobari"
		fmt.Fprintf(&addSourceCalls, `
	{
		_c, _ := %[1]s_embeddedSources.ReadFile(%[2]q)
		%[1]s_AddEmbeddedSource(%[3]q, _c)
	}`, tobariPkg, tobariName, inputFile)
	}
...
//go:embed *.go.tobari
var %[2]s_embeddedSources embed.FS
...
func %[2]s_AddEmbeddedSource(string, []byte) bool

var _ = %[2]s_SetGIDFunc(func() uint64 { return %[2]s_GID() })
var _ = func() bool {%[3]s
	return true
}()
`, pkgcfg.PkgName, tobariPkg, addSourceCalls.String())
	return os.WriteFile(covervarsPath, []byte(src), 0o600)
}
