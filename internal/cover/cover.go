...
func Run(ctx context.Context, args []string) error {
	...
	if len(inputFiles) == 1 && opt.output != "" {
		...
		if opt.outputFileList != "" {
			if err := writeOutputFileList(opt.outputFileList, []string{opt.output}); err != nil {
				return err
			}
		}
		if err := createCovervars(pkgcfg, opt.pkgcfg); err != nil {
			return err
		}
		return nil
	}
...
func createCovervars(pkgcfg *PackageConfig, pkgcfgPath string) error {
	if pkgcfgPath != "" {
