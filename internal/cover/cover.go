...
func Run(ctx context.Context, args []string) error {
	...
	if len(inputFiles) == 1 && opt.output != "" {
		...
		if err := createCovervars(pkgName, opt.pkgcfg); err != nil {
			return nil
		}
		return nil
	}
...
func createCovervars(pkgName, pkgcfg string) error {
	if pkgcfg != "" {
...
func AddTracePointWithContent(filename string, content []byte, mode string) ([]byte, error) {
	...
	return append(file.edit.Bytes(), []byte(file.renderFooter())...), nil
}
...
func (f *File) renderFooter() string {
	footer := "\n"
	footer += fmt.Sprintf(`var _ = %s.SetGIDFunc(func() uint64 { return %s.GID() })`, tobariPkg, runtimePkg) + "\n"
	footer += fmt.Sprintf(`var _ = %s.AddCoverMeta(%s)`, tobariPkg, f.renderMetadata()) + "\n"
	return footer
}

func (f *File) renderMetadata() string {
	return fmt.Sprintf(`%s.Metadata{FileName: "%s", Funcs: %s}`, tobariPkg, f.name, f.renderFunctions(f.funcs))
}
...
func (f *File) renderBlocks(blocks []*tobari.Block) string {
	rendered := make([]string, 0, len(blocks))
	for _, block := range blocks {
		rendered = append(rendered,
			fmt.Sprintf(`{Idx: %d, Start: %s, End: %s, NumStmts: %d}`, block.Idx, f.renderPos(block.Start), f.renderPos(block.End), block.NumStmts),
		)
	}
...
func (f *File) renderPos(pos tobari.Pos) string {
	return fmt.Sprintf(`%s.Pos{Line: %d, Col: %d}`, tobariPkg, pos.Line, pos.Col)
}

func (f *File) addImport() {
	// Add import after package declaration
	importStmt := f.renderImports()
	f.edit.Insert(f.offset(f.astFile.Name.End()), importStmt)
}

func (f *File) renderImports() string {
	imprt := "\n"
	imprt += fmt.Sprintf(`import %s "runtime"`, runtimePkg) + "\n"
	imprt += fmt.Sprintf(`import %s "github.com/goccy/tobari"`, tobariPkg) + "\n"
	return imprt
}
...
func (f *File) newCounter(start, end token.Pos, numStmt int) string {
	...
	stmt := fmt.Sprintf(
		"%s.Trace(%q, %s.PGID(), %s.GID(), %d, %d, %d, %d, %d, %d)",
		tobariPkg,
		f.name,
		runtimePkg,
		runtimePkg,
